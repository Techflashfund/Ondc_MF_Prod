...
from .utils import build_frequency, get_client_ip,push_observability_logs,send_to_analytics
...
class OnSearchView(APIView):
    ...
    "message": {
        "ack": {"status": "NACK"}
            ...
            try:
                isin = (
                    data["message"]["catalog"]["providers"][0]["items"][1]["tags"][1]["list"][0]["value"]
                )
          ...
          "stops": [
            {
              "time": {
                "schedule": {
                  "frequency":build_frequency(frequency=frequency, repeat=repeat, day_number=date)
                }
              }
            }
          ]
...
class INIT(APIView):
    ...
    def post(self,request,*args,**kwargs):
        ...
        obj=get_object_or_404(SelectSIP,payload__context__bpp_id=bpp_id,payload__context__bpp_uri=bpp_uri,transaction__transaction_id=transaction_id,payload__context__message_id=message_id_select)
        if not message_id:       
                message_id = str(uuid.uuid4())
        timestamp = datetime.utcnow().isoformat(sep="T", timespec="milliseconds") + "Z"
...
class ConfirmSIP(APIView):
    def post(self,request,*args,**kwargs):
        ...
        obj=get_object_or_404(OnInitSIP,payload__context__bpp_id=bpp_id,payload__context__bpp_uri=bpp_uri,transaction__transaction_id=transaction_id,payload__context__message_id=message_id_init)
        if not message_id:
             message_id = str(uuid.uuid4())
        timestamp = datetime.utcnow().isoformat(sep="T", timespec="milliseconds") + "Z"
...
class DigiLockerFormSubmission(APIView):
        ...
        def post(self,request,*args,**kwargs):
            ...
            obj=OnStatus.objects.filter(transaction__transaction_id=transaction_id).order_by('-timestamp').first()
            if not message_id:
                    message_id = str(uuid.uuid4())
            timestamp = datetime.utcnow().isoformat(sep="T", timespec="milliseconds") + "Z"
...
class SIPExixstingInit(APIView):
    def post(self,request,*args,**kwargs):
        ...
        if not all([transaction_id, bpp_id, bpp_uri,message_id,phone]):
            return Response({"error": "Missing transaction_id, bpp_id, or bpp_uri,message_id,phone"}, 
                          status=status.HTTP_400_BAD_REQUEST)
                    ...
                    "message": {
                        ...
                        "fulfillments": [
                            ...
                            "customer": {
                                ...
                                "creds": [
                                    {
                                    "id": fulfillments[0]['tags'][1]['list'][0]['value'],
                                    "type": "FOLIO"
                                    },
                                    {
                                    "id": get_client_ip(request),
                        ...
                        "payments": [
                            ...
                            "params": {
                                "amount": item[0]['quantity']['selected']['measure']['value'],
                                "currency": item[0]['quantity']['selected']['measure']['unit'],
...
class SIPExistingConfirm(APIView):
    def post(self,request,*args,**kwargs):
        ...
        obj=get_object_or_404(OnInitSIP,payload__context__bpp_id=bpp_id,payload__context__bpp_uri=bpp_uri,transaction__transaction_id=transaction_id)
                    ...
                    "fulfillments": [
                        ...
                        "customer": {
                            ...
                            "creds": [
                                ...
                                {
                                "id": fulfillments[0]['customer']['person']['creds'][1]['id'],
                                "type": "IP_ADDRESS"
                                }
                            ]
                            },
...
class StatusAPIView(APIView):
  ...
  "context": {
    ...
    "action": "status"
        ...
        response = requests.post(f"{bpp_uri}/status", data=request_body_str, headers=headers) 
        try:
...
class Lumpsum(APIView):
    ...
    def post(self,request,*args,**kwargs):
        ...
        matching_fulfillment = next((f for f in provider[0]["fulfillments"] if f.get("type") == preferred_type),
        None
    )
                        ...
                        "payments": [
                            ...
                            "tags": [
                                ...
                                "list": [
                                    ...
                                    "value": payment[0]['tags'][0]['list'][0]['value']
                                    }
                                ]
                                }
                            ]
                ...
                "message": {
                    ...
                    "payments": [
                        ...
                        "params": {
                            ...
                            "source_bank_code": payment[0]['params']['source_bank_code'],
                            "source_bank_account_number": payment[0]['params']['source_bank_account_number'],
                            "source_bank_account_name": payment[0]['params']['source_bank_account_name']
                        },
                        "type": payment[0]['type'],
                        "tags": [
                            {
                ...
                "fulfillments": [
                    ...
                    "tags": [
                        ...
                        "list": [
                            ...
                            "value": fulfillments[0]['tags'][1]['list'][0]['value']
                            }
                        ]
                        }
                    ]
                    ...
                    "fulfillments": [
                        ...
                        "tags": [
                            ...
                            "list": [
                                ...
                                "value": fulfillments[0]['tags'][0]['list'][0]['value']
                                }
                            ]
                            }
                        ]
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if not all ([transaction_id,message_id]):
                return Response({"error": "Missing transaction_id,message_id"}, status=status.HTTP_400_BAD_REQUEST)

            queryset = SelectSIP.objects.filter(transaction__transaction_id=transaction_id,message_id=message_id)
...
class OnInitDataView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            message_id = request.data.get('message_id')
            if not all([transaction_id, message_id]):
                return Response({"error": "Missing transaction_id or message_id"}, 
                            status=status.HTTP_400_BAD_REQUEST)
            obj = get_object_or_404(
                OnInitSIP,
...
class OnConfirmDataView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if not all([transaction_id, message_id]):
                return Response({"error": "Missing transaction_id, bpp_id, bpp_uri or message_id"}, 
                            status=status.HTTP_400_BAD_REQUEST)
...
class OnStatusDataView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            obj = (
                OnStatus.objects
                .filter(transaction__transaction_id=transaction_id)
                .order_by('-timestamp')
                .first()
            )
...
class OnStatusListPan(APIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            pan = request.data.get('pan')
            if not pan:
                return Response({"error": "Missing pan"}, status=status.HTTP_400_BAD_REQUEST)
            
            obj = OnStatus.objects.filter(pan=pan).order_by('-timestamp').first()
            if not obj:
                return Response({"error": "No records found for the given PAN"}, status=status.HTTP_404_NOT_FOUND)
            return Response({
                "status": "success",
