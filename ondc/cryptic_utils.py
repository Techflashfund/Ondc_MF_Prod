...
def hash_message(msg: str):
    ...
    digest = HASHER(
        bytes(msg, "utf-8"), digest_size=64, encoder=nacl.encoding.Base64Encoder
    )
...
def verify_response(signature, signing_key, public_key):
    try:
        ...
        VerifyKey(public_key64).verify(
            bytes(signing_key, "utf8"), base64.b64decode(signature)
        )
        return True
    except Exception:
        return False
...
def get_filter_dictionary_or_operation(filter_string):
    ...
    return {
        kv.split("=")[0].strip(): kv.split("=")[1].strip().replace('"', "")
        for kv in filter_string_list
    }
...
def verify_authorisation_header(auth_header, request_body_str=None, public_key=None):
    ...
    header_parts = get_filter_dictionary_or_operation(
        auth_header.replace("Signature ", "")
    )
    ...
    signing_key = create_signing_string(
        hash_message(request_body_str), created=created, expires=expires
    )
    return verify_response(
        header_parts["signature"], signing_key, public_key=public_key
    )
...
def generate_key_pairs():
    ...
    encryption_private_key = base64.b64encode(
        inst_private_key.private_bytes(
...
def encrypt(encryption_private_key, encryption_public_key, _=None):
    private_key = serialization.load_der_private_key(
        base64.b64decode(encryption_private_key), password=None
    )
